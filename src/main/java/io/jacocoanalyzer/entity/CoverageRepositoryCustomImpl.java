...
public class CoverageRepositoryCustomImpl implements CoverageRepositoryCustom {
  ...
  @Override public List<ReportSumCoverage> sumByPackage() {
    ...
    String COV_GROUP_QUERY = "select row_number() over (order by report_name) as id, -- as per_report\n"
        + "       coverage_run_name,\n" + "       report_name,\n"
        ...
        + "                         sum(lines_missed)                                                                 as lines_missed,\n"
        + "                         sum(complexity_missed + complexity_covered)                                       as comp,\n"
        + "                         sum(cast(lines_covered as float) / cast((lines_missed + lines_covered) as float)) as cov,\n"
        + "                         sum(lines_missed + lines_covered)                                                 as lines\n"
